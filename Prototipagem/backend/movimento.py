...
def calcular_espaco_tempo(data_points):  # Função que calcula o tempo (em segundos) e o espaço percorrido com base nos pontos GPS
    ...
    for i, ponto in enumerate(data_points):  # Itera sobre os pontos de dados GPS
        ts = datetime.fromisoformat(ponto['timestamp'].replace("Z", "+00:00"))  # Converte timestamp ISO 8601 para datetime
        if i > 0:  # A partir do segundo ponto, calcula distância em relação ao ponto anterior
    ...
    tempos_segundos = [(t - tempo0).total_seconds() for t in tempos]  # Lista com tempos relativos em segundos

    return np.array(tempos_segundos), np.array(espacos)  # Retorna dois arrays: tempos e espaços


def calcular_movimento(tempos, espacos):  # Função para calcular velocidades e acelerações a partir de tempos e espaços
    velocidades = np.diff(espacos) / np.diff(tempos)  # Calcula velocidades instantâneas (diferença do espaço / diferença do tempo)
    aceleracoes = np.diff(velocidades) / np.diff(tempos[:-1])  # Calcula acelerações instantâneas (diferença da velocidade / diferença do tempo)

    velocidade_media = (espacos[-1] - espacos[0]) / (tempos[-1] - tempos[0])  # Velocidade média total (deslocamento / tempo total)

    if len(velocidades) > 0:  # Se houver velocidades calculadas
        aceleracao_media = (velocidades[-1] - velocidades[0]) / (tempos[-1] - tempos[0])  # Aceleração média total (variação da velocidade / tempo total)
    else:
